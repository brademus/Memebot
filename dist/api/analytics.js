...
async function buildAnalytics() {
    const mostActiveNow = (0, store_1.activeTokens)()
    ...
    if (!db_1.pool)
        return { mostActiveNow, topWallets: [], topCoinsToday: [], todayStats: null, note: 'attach Postgres for wallet + outcome analytics' };
    const q = async (sql) => (await db_1.pool.query(sql)).rows;
