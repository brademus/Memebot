...
exports.startDexscreenerPoller = startDexscreenerPoller;
exports.fetchTokenSnapshot = fetchTokenSnapshot;
...
function startDexscreenerPoller(onUpdated) {
    const tick = async () => {
        const tracked = (0, store_1.allTokens)().filter(t => t.state !== 'DEAD');
        const batchSize = (0, config_1.cfg)().limits.dexscreener_batch_size;
        for (let i = 0; i < tracked.length; i += batchSize) {
            const batch = tracked.slice(i, i + batchSize);
            await enrich(batch, onUpdated);
        }
        setTimeout(tick, (0, config_1.cfg)().polling.dexscreener_interval_ms);
    };
    tick();
}
async function enrich(batch, onUpdated) {
    if (!batch.length)
        return;
    try {
...
async function fetchTokenSnapshot(ca) {
    try {
