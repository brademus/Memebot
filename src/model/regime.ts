...
let current: MarketRegime = {
  ...
  routeHealth: 0, changeProbability: 1, completeness: 0,
};
...
export function classifyRegime(metrics: Omit<MarketRegime, 'id' | 'kind' | 'observedAt' | 'changeProbability'>): RegimeKind {
  ...
  if (metrics.launches1h < 250 || metrics.passRate < 0.12 || metrics.aggregateBuyRatio < 0.9) return 'cold';
  return 'normal';
}

export function startRegimeEngine() {
  const tick = () => refreshRegime().catch(error => {
    diag.lastError = (error as Error).message;
    console.error('[regime]', diag.lastError);
  });
...
export async function refreshRegime(): Promise<MarketRegime> {
  const live = allTokens().filter(token => token.firstSeen >= Date.now() - 60 * 60_000);
  ...
  if (pool) {
    const database = await pool.query(
      `SELECT COUNT(*)::int AS launches,
              COALESCE(AVG(CASE WHEN gate_result='passed' THEN 1.0 ELSE 0 END),0) AS pass_rate
         FROM tokens WHERE first_seen>now()-interval '1 hour'`,
    ).catch(() => ({ rows: [] as any[] }));
    if (database.rows.length) {
      launches1h = Number(database.rows[0].launches) || launches1h;
      passRate = Number(database.rows[0].pass_rate) || passRate;
    }
    const routes = await pool.query(
      `SELECT COALESCE(AVG(CASE WHEN eligible AND simulation_ok THEN 1.0 ELSE 0 END),0) AS route_health
         FROM execution_probes WHERE probed_at>now()-interval '1 hour'`,
    ).catch(() => ({ rows: [] as any[] }));
    routeHealth = Number(routes.rows[0]?.route_health) || 0;
  }

  const metrics = {
    launches1h,
    passRate: clamp01(passRate),
    medianChange5m: percentile(changes, 0.5),
    aggregateBuyRatio: sells > 0 ? buys / sells : buys > 0 ? 3 : 1,
    medianLiquidityUsd: percentile(liquidity, 0.5),
    routeHealth,
    completeness: clamp01(0.35 * Math.min(1, live.length / 100) + 0.25 * (changes.length ? 1 : 0)
      + 0.20 * (liquidity.length ? 1 : 0) + 0.20 * (routeHealth > 0 ? 1 : 0)),
  };
  const provisionalKind = classifyRegime(metrics);
  const previous = history[history.length - 1];
  const changeProbability = previous ? regimeDistance(previous, metrics) : 1;
  const kind: RegimeKind = previous && changeProbability > 0.75 && provisionalKind !== previous.kind ? 'transition' : provisionalKind;
  const observedAt = Date.now();
  current = {
    id: `${new Date(observedAt).toISOString().slice(0, 13)}:${kind}`,
    kind, observedAt, launches1h,
    ...
    routeHealth: round(metrics.routeHealth), changeProbability: round(changeProbability),
    completeness: round(metrics.completeness),
  };
  ...
  if (pool) await pool.query(
    `INSERT INTO regime_snapshots
       (regime_id,kind,observed_at,launches_1h,pass_rate,median_change_5m,aggregate_buy_ratio,
        median_liquidity_usd,route_health,change_probability,completeness,metrics)
     VALUES ($1,$2,to_timestamp($3/1000.0),$4,$5,$6,$7,$8,$9,$10,$11,$12)`,
    [current.id, current.kind, current.observedAt, current.launches1h, current.passRate,
     current.medianChange5m, current.aggregateBuyRatio, current.medianLiquidityUsd,
     current.routeHealth, current.changeProbability, current.completeness, JSON.stringify(current)],
  ).catch(() => {});
  return current;
}
