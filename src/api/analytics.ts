...
export async function buildAnalytics(): Promise<any> {
  const mostActiveNow = activeTokens()
    ...
    .slice(0, 10)
    .map(token => ({
      ca: token.ca, symbol: token.symbol, state: token.state, score: token.score,
      vol5m: Math.round(token.vol5m), txns5m: token.buys5m + token.sells5m,
  ...
  const topWallets = await query(`
    SELECT w.wallet,w.winners_hit,w.active,w.quality_verdict,w.win_rate,w.round_trips,w.discovered_from,
           ...
           COUNT(DISTINCT h.ca) AS buys_tracked,
           COUNT(DISTINCT o.ca) FILTER (WHERE o.multiple_from_buy >= 2) AS wins_2x,
           ROUND(AVG(o.multiple_from_buy)::numeric,2) AS avg_multiple
      FROM smart_wallets w
      LEFT JOIN wallet_hits h ON h.wallet=w.wallet
      LEFT JOIN wallet_hit_outcomes o
        ON o.wallet=w.wallet AND o.ca=h.ca AND o.snapshot_minutes=240
     ...
     ORDER BY (w.last_active > now()-interval '24 hours') DESC,
              w.active DESC,(w.quality_verdict='ELITE') DESC,
              wins_2x DESC NULLS LAST,w.winners_hit DESC
     LIMIT 25`);
  ...
  const topCoinsToday = await query(`
  ...
  return {
    mostActiveNow, topWallets: enrichedWallets, topCoinsToday, todayStats,
