...
const diag = { refreshes: 0, loaded: 0, lastRefreshAt: null as string | null, lastSuccessAt: null as string | null, lastError: null as string | null };
export const entryRevalidationDiag = () => ({ enabled: !!pool, refreshIntervalSeconds: REFRESH_MS / 1000, references: references.size, ...diag });
...
async function refreshQualityReferences() {
  ...
  try {
    const result = await pool.query(`SELECT DISTINCT ON (ca) id,ca,signal,entry_at,entry_price
      FROM paper_trades WHERE strategy_role='quality_observation' AND strategy_version=$1
      AND entry_at>now()-interval '48 hours' ORDER BY ca,entry_at DESC`, [STRATEGY_VERSION]);
    ...
    for (const row of result.rows) {
      ...
      next.set(String(row.ca), { paperTradeId: Number(row.id), ca: String(row.ca), signal: String(row.signal || 'quality'), selectedAt, selectedPrice });
    }
    references.clear();
...
const memoryOnly = (): EntryRevalidationAssessment => ({
  revalidationReady: true, revalidationBlockers: [],
  revalidationReasons: ['Quality-reference enforcement is inactive without PostgreSQL.'],
  qualityReference: null, selectionPremiumPct: null, liquidityReady: true,
  retentionReady: true, marketContinuityReady: true, qualityTooLate: false,
  earlyRetention: null, movedFromFirstScorePct: null,
});

export function assessEntryRevalidation(token: TokenRecord, now = Date.now(), referenceOverride?: QualityReference | null): EntryRevalidationAssessment {
  if (!pool && referenceOverride === undefined) return memoryOnly();
  ...
  const marketContinuityReady = token.source === 'aged' || movedFromFirstScorePct === null || movedFromFirstScorePct > -80;

  if (!reference || now - reference.selectedAt > MAX_REFERENCE_AGE_MS) blockers.push('quality selection reference unavailable or stale');
  ...
  else if (movedFromFirstScorePct !== null) reasons.push(`Price continuity from first score is ${movedFromFirstScorePct.toFixed(1)}%.`);

  return { revalidationReady: blockers.length === 0, revalidationBlockers: blockers, revalidationReasons: reasons,
    qualityReference: reference, selectionPremiumPct, liquidityReady, retentionReady, marketContinuityReady,
    qualityTooLate, earlyRetention: retention, movedFromFirstScorePct };
}
