...
export function benchmarkExitDecision(entryPrice: number, markPrice: number, peakPrice: number, ageHours: number): StrategyExitEvaluation {
  ...
  const peak = Math.max(mark, positive(peakPrice));
  ...
  const common = { peakMultiple, activeStopMultiple: ADAPTIVE_EXIT_POLICY.hardStopMultiple, deteriorationSignals: [], metrics };

  if (peakMultiple >= ADAPTIVE_EXIT_POLICY.takeProfitMultiple) return {
    ...
    multiple: ADAPTIVE_EXIT_POLICY.takeProfitMultiple, ...common,
  };
  if (multiple <= ADAPTIVE_EXIT_POLICY.hardStopMultiple) return {
...
export function adaptiveExitDecision(input: StrategyExitInput): StrategyExitEvaluation {
  ...
  const peak = Math.max(mark, positive(input.peakPrice));
  const multiple = entry > 0 && mark > 0 ? mark / entry : 0;
  const peakMultiple = entry > 0 && peak > 0 ? peak / entry : multiple;
  const buySellRatio = input.sells5m > 0 ? input.buys5m / input.sells5m : input.buys5m > 0 ? 3 : 1;
  const scoreDrop = input.entryScore !== null && input.currentScore !== null ? input.entryScore - input.currentScore : 0;
  ...
  let activeStopMultiple: number = ADAPTIVE_EXIT_POLICY.hardStopMultiple;
  if (peakMultiple >= ADAPTIVE_EXIT_POLICY.secondProfitArmMultiple) {
    activeStopMultiple = Math.max(ADAPTIVE_EXIT_POLICY.secondProfitFloorMultiple,
      peakMultiple * (1 - ADAPTIVE_EXIT_POLICY.secondProfitTrailPct));
  } else if (peakMultiple >= ADAPTIVE_EXIT_POLICY.firstProfitArmMultiple) {
    activeStopMultiple = Math.max(ADAPTIVE_EXIT_POLICY.firstProfitFloorMultiple,
  ...
  if (input.modelDownsideProbability !== null && input.modelDownsideProbability > ADAPTIVE_EXIT_POLICY.modelHighDownsideProbability)
    deteriorationSignals.push(`model downside probability rose to ${(input.modelDownsideProbability * 100).toFixed(1)}%`);
  if (input.state === 'DYING' || input.state === 'DEAD') deteriorationSignals.push(`token state changed to ${input.state}`);
  ...
  const metrics: Record<string, number | string | boolean | null> = {
    ...
    liquidityDropPct, buys5m: input.buys5m, sells5m: input.sells5m, buySellRatio,
    entrySmartWallets: input.entrySmartWallets, currentSmartWallets: input.currentSmartWallets,
  ...
  if (peakMultiple >= ADAPTIVE_EXIT_POLICY.takeProfitMultiple)
    return result('sell', 'strategy_take_profit_3x', ['The timed paper entry reached the 3x profit objective.'],
      entry * ADAPTIVE_EXIT_POLICY.takeProfitMultiple, ADAPTIVE_EXIT_POLICY.takeProfitMultiple);
  if (multiple <= ADAPTIVE_EXIT_POLICY.hardStopMultiple)
    return result('sell', 'strategy_hard_stop_50pct', ['The timed paper entry reached the maximum 50% loss.'],
      entry * ADAPTIVE_EXIT_POLICY.hardStopMultiple, ADAPTIVE_EXIT_POLICY.hardStopMultiple);
  if (input.insiderKilled || input.fundedSnipers > 0)
    return result('sell', 'strategy_insider_risk_exit', [input.insiderKilled
      ? 'Late evidence marked the token as insider-controlled.'
      : 'Deployer-linked funded snipers appeared after entry.'], mark);
  if (liquidityDropPct >= ADAPTIVE_EXIT_POLICY.liquidityEmergencyDropPct)
    return result('sell', 'strategy_liquidity_emergency_exit',
      [`Liquidity fell ${(liquidityDropPct * 100).toFixed(1)}% from the entry snapshot.`], mark);
  if (peakMultiple >= ADAPTIVE_EXIT_POLICY.firstProfitArmMultiple && multiple <= activeStopMultiple)
    return result('sell', peakMultiple >= ADAPTIVE_EXIT_POLICY.secondProfitArmMultiple
