...
export async function openPaper(
  ...
  const inserted = await pool.query(
    `INSERT INTO paper_trades
       (ca, symbol, signal, entry_price, mark_entry_price, entry_score,
        ...
        model_version, quote_key_present)
     VALUES ($1,$2,$3,$4,$4,$5,$4,now(),$4,now(),$6,'quote_pending',$7,$8)
  ...
  if (!token) {
    await pool.query(
      `UPDATE paper_trades
          SET quote_status='token_not_in_memory', quote_attempted_at=now(), quote_key_present=$3
        WHERE ca=$1 AND signal=$2`,
      [ca, signal, keyPresent],
    ).catch(() => {});
    return;
  }
  ...
  await pool.query(
    `UPDATE paper_trades
        SET entry_price=CASE WHEN $3 THEN $4 ELSE entry_price END,
            ...
            quote_status=$5,
            quote_attempted_at=now(),
            quote_key_present=$15,
            position_sol=$6,
            position_usd=$7,
            quoted_out_usd=$8,
            quoted_out_amount=$9,
    ...
    [
      ...
      quote.priceImpact, quote.slippageBps, quote.feeLamports, quote.router,
      quote.quoteTimeMs, keyPresent,
    ],
  ).catch(() => {});
}
  ...
  for (const row of open.rows) {
      ...
      if (!row.execution_eligible) {
        ...
        await pool.query(
          ...
          [
            ...
            exitQuote.priceImpact, exitQuote.feeLamports, exitQuote.router,
            exitQuote.quoteTimeMs,
          ],
        ).catch(() => {});
    ...
    if (verifiedTarget) {
      ...
      await closeAt(
        ...
        row.signal,
        verifiedExitPrice,
        row.execution_eligible ? 'target_3x_exit_verified' : 'target_3x_observed_legacy',
      );
      continue;
    }
...
export async function paperScoreboard(days = 30): Promise<any[]> {
  ...
  const result = await pool.query(
    `SELECT signal, model_version,
      ...
      GROUP BY signal, model_version
      ORDER BY model_version DESC, pct_3x_executable DESC NULLS LAST`,
    [String(days)],
  ).catch(() => null);
  ...
  return (result?.rows || []).map((row: any) => {
    ...
    return {
      ...row,
      current_model: row.model_version === MODEL_VERSION,
...
export async function paperQuoteStatusBreakdown(days = 30): Promise<any[]> {
  if (!pool) return [];
  const result = await pool.query(
    `SELECT model_version, quote_status, quote_key_present, COUNT(*)::int AS n,
            MIN(entry_at) AS first_at, MAX(entry_at) AS last_at
       FROM paper_trades
      WHERE entry_at > now()-($1||' days')::interval
      GROUP BY model_version, quote_status, quote_key_present
      ORDER BY model_version DESC, n DESC`,
    [String(days)],
  ).catch(() => ({ rows: [] as any[] }));
  return result.rows.map((row: any) => ({
    ...row,
