...
test('stale in-memory prices are not live observations (production function)', async () => {
  const { isFreshPriceObservation } = await import('./paper');
  const now = Date.now();
  assert.equal(isFreshPriceObservation({ priceUsd: 0.001, priceAt: now - 60_000 }, now), true);
  assert.equal(isFreshPriceObservation({ priceUsd: 0.001, priceAt: now - 31 * 60_000 }, now), false);
  assert.equal(isFreshPriceObservation({ priceUsd: 0.001 }, now), false, 'no observation timestamp means not alive');
  assert.equal(isFreshPriceObservation(null, now), false);
});
