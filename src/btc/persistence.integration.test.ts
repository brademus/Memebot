...
test('BTC paper schema is idempotent and enforces one-open and daily limits', async () => {
  assert.ok(pool, 'DATABASE_URL must be configured for the BTC persistence integration test');
  const schema = fs.readFileSync(path.join(process.cwd(), 'schema-btc.sql'), 'utf8');

  await pool.query('DROP TABLE IF EXISTS btc_calls CASCADE');
  await pool.query('DROP FUNCTION IF EXISTS btc_enforce_daily_paper_limits() CASCADE');
  await pool.query(schema);
  await pool.query(schema);

  const insert = async (id: string, openedAt: string) => pool.query(
  ...
  const rows = await pool.query(`SELECT id,status,result_r FROM btc_calls ORDER BY opened_at`);
